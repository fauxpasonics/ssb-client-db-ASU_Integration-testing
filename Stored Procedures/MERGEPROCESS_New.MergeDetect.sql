...
--[MERGEPROCESS_New].[MergeDetect]  'ASU'
	@Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account varchar(100) = (Select CASE WHEN @client = 'TrailBlazers' THEN 'CRM_Account' ELSE Concat(@client,' PC_SFDC Account' ) END); --updateme
