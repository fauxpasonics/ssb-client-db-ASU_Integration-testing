...
CREATE	 VIEW  [dbo].[vwCRMLoad_Custom_TM_Transaction__c]
AS 
...
INNER JOIN dbo.vwCRMProcess_DeDupProdCopyAcct_ByGUID a WITH (NOLOCK) ON dimcust.SSB_CRMSYSTEM_CONTACT_ID = a.SSB_CRMSYSTEM_CONTACT_ID__c AND a.Rank = 1 --updateme based on Account model
LEFT JOIN ASU_Reporting.prodcopy.TM_Transactions__c pctt WITH (NOLOCK) ON pctt.FactTicketSalesID__c = t.FactTicketSalesID__c --updateme for TM_Transaction (only because ASU had both Pac and TM)
...
WHERE 1=1 AND (pctt.id IS NULL
	...
	OR ISNULL(pctt.AccountId__c,'') != ISNULL(a.Id,'')
