...
CREATE VIEW [dbo].[vwCRMLoad_Custom_Transaction__c_OldModel]
AS 
...
INNER JOIN dbo.[vwDimCustomer_ModAcctId] dimcust WITH (NOLOCK) ON t.patron_id__c = [dimcust].[SSID] AND [dimcust].[SourceSystem] = 'TI ASU' --UPDATEME
...
LEFT JOIN prodcopy.Transactions__c pctt WITH (NOLOCK) ON pctt.order_line_id__c COLLATE SQL_Latin1_General_CP1_CS_AS = t.order_line_id__c 
